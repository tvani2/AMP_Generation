...
# Resolve data relative to the repository root (…/data), not the CWD.
_PROJECT_ROOT = Path(__file__).resolve().parents[2]
_DATA_DIR = _PROJECT_ROOT / "data"
_VOCAB_PATH = _DATA_DIR / "peptide_vocab.pkl"
_WEIGHT_PATH = _DATA_DIR / "peptide_weight.npy"

if not _VOCAB_PATH.exists() or not _WEIGHT_PATH.exists():
    raise FileNotFoundError(
        "TransVAE vocab files not found.\n"
        f"  Expected: {_VOCAB_PATH}\n"
        f"            {_WEIGHT_PATH}\n"
        "Place peptide_vocab.pkl and peptide_weight.npy under data/ "
        "(see data/README.md)."
    )

with open(_VOCAB_PATH, "rb") as f:
    w2i = pickle.load(f)
char_weights = np.load(_WEIGHT_PATH)
org_dict = {}
